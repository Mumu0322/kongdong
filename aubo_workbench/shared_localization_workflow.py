...
def _batch_coarse_localization_at_340mm(
    ...
    *,
    ...
    use_projected_anchor_correction: bool = True,
    progress_callback: Any | None = None,
) -> dict[Any, dict[str, Any]]:
    """在配置的共同观察高度一次性检测所有孔的位姿和深度。
    ...
    capture_height_mm = float(getattr(cfg, "coarse_height_mm", 340.0))
    height_label = f"{capture_height_mm:g}"
    direct_strategy = bool(getattr(cfg, "coarse_direct_final", False))
    capture_mode = (
        f"batch_coarse_at_{height_label}mm_pointcloud_only"
        if direct_strategy else f"batch_coarse_at_{height_label}mm"
    )
    ...
    max_consecutive_frame_failures = (
        max(1, int(getattr(cfg, "coarse_direct_final_max_consecutive_frame_failures", 3)))
        if direct_strategy else None
    )
    settle_discard_timed_out = False
    capture_timed_out = False
    max_consecutive_frame_failures_reached = False
    ...
    anchor_correction_info: dict[str, Any] | None = (
        None
        if use_projected_anchor_correction
        else {
            "enabled": False,
            "reason": "disabled_for_coarse_direct_pointcloud_only",
        }
    )
    anchor_correction_attempted = not use_projected_anchor_correction
    ...
    def notify_progress(phase: str, **details: Any) -> None:
        ...
        if not callable(callback):
            return
        payload = {"phase": str(phase), **details}
        try:
            callback(payload)
        except Exception:
            # Progress reporting is diagnostic only and must never stop capture.
            return
    ...
    if settle_discard_frames > 0:
        settle_deadline = (
            time.monotonic() + float(settle_discard_timeout_s)
            if settle_discard_timeout_s is not None else None
        )
        notify_progress(
            "discard_settle",
            target_frames=int(settle_discard_frames),
            timeout_s=settle_discard_timeout_s,
        )
        with timing.measure(
            ...
            with visual_measure(
                timing,
                "batch_coarse/frame_acquisition_settle",
                "frame_acquisition",
                target_frames=int(settle_discard_frames),
            ):
                for discard_index in range(settle_discard_frames):
                    if settle_deadline is not None and time.monotonic() >= settle_deadline:
                        settle_discard_timed_out = True
                        notify_progress(
                            "discard_settle_timeout",
                            discarded_frame_count=int(discarded_frame_count),
                        )
                        break
    ...
    with timing.measure(
        ...
        capture_deadline = (
            time.monotonic() + float(capture_timeout_s)
            if capture_timeout_s is not None else None
        )
        notify_progress(
            "capture_start",
            target_frames=int(batch_frames),
        ...
        for frame_index in range(batch_frames):
            if capture_deadline is not None and time.monotonic() >= capture_deadline:
                ...
                capture_stop_reason = "capture_timeout"
                notify_progress(
                    "capture_timeout",
            ...
            with visual_measure(
                timing,
                "batch_coarse/frame_acquisition",
                "frame_acquisition",
                frame_index=int(frame_index),
            ):
                bundle = get_aligned_frame_bundle(pipeline, align, chain)
            if bundle is None or bundle.intrinsics is None:
                ...
                notify_progress(
                    ...
                    captured_frame_count=len(frame_records),
                    valid_hole_count=0,
                    consecutive_frame_failures=int(consecutive_frame_failures),
                )
                if (
                    max_consecutive_frame_failures is not None
                    and consecutive_frame_failures >= max_consecutive_frame_failures
                ):
                    max_consecutive_frame_failures_reached = True
                    capture_stop_reason = "consecutive_frame_failures"
                    notify_progress(
                        "capture_stop_consecutive_failures",
                        frame_index=int(frame_index),
                        consecutive_frame_failures=int(consecutive_frame_failures),
                    )
                    break
            ...
            try:
                with visual_measure(
                    timing,
                    "batch_coarse/yolo_inference",
                    "yolo_inference",
                    frame_index=int(frame_index),
                ):
                    detections = detect(model, bundle.color_bgr, confidence)
                if use_projected_anchor_correction and not anchor_correction_attempted:
            ...
            for hole_id, hole in holes_by_id.items():
                ...
                try:
                    ...
                    observation = Observation(
                        ...
                        quality_note="pointcloud_center_from_yolo_ray",
            ...
            frame_is_valid = valid_hole_count >= min_holes_per_frame
            if frame_is_valid:
                consecutive_frame_failures = 0
            else:
                # Partial YOLO/点云结果也属于整组无效帧；否则连续“只拍到
                # 一部分孔”的情况会绕过失败上限，直到耗尽全部采集帧。
                consecutive_frame_failures += 1
            ...
            cv2.putText(
                view,
                f"Batch RGB-D {height_label}mm frame={frame_index} "
                f"valid={valid_hole_count}/{len(selected_holes)}",
            ...
            if bool(getattr(cfg, "save_all_capture_overlays", False)):
                frame_overlay_path = run_dir / (
                    f"{artifact_tag}batch_coarse_{height_label}_frame_{frame_index:02d}.png"
                )
    ...
    if latest_view is not None and not bool(
        ...
        latest_overlay_path = run_dir / (
            f"{artifact_tag}batch_coarse_{height_label}_last_frame.png"
        )
        with artifact_measure(
            timing,
    ...
    for hole in selected_holes:
        ...
        if diagnostic is not None:
            diagnostic_path = run_dir / f"{artifact_tag}hole_{hole_id:02d}_surface_diagnostic.npz"
            with artifact_measure(timing, "batch_coarse/write_surface_diagnostic",
        ...
        rows.extend(dict(row, hole_id=hole_id, capture_tag=artifact_tag)
                    for row in _observation_rows(observations))

        if len(valid_observations) < min_valid:
            # 当前孔的有效帧数不足，标记为失败
            batch_results[hole_id] = {
                "success": False,
                "capture_height_mm": capture_height_mm,
        ...
        try:
            with visual_measure(
                timing,
                f"batch_coarse/fusion_quality_hole_{hole_id:02d}",
                "fusion_quality",
                hole_id=int(hole_id),
            ):
            ...
            with visual_measure(
                timing,
                f"batch_coarse/coordinate_transform_hole_{hole_id:02d}",
            ...
            batch_results[hole_id] = {
                ...
                "ring_coverage_min_ratio": summary.get("ring_coverage_min_ratio"),
                "ring_max_gap_deg": summary.get("ring_max_gap_deg"),
                "ring_quality_rejected_frames": summary.get("ring_quality_rejected_frames", 0),
                "ring_points_median": summary.get("ring_points_median"),
                "center_px": summary["center_px"],
                "center_source": summary.get("center_source"),
                "center_source_counts": summary.get("center_source_counts", {}),
                "geometric_valid_frames": summary.get("geometric_valid_frames", 0),
                "geometric_center_scatter_p95_px": summary.get(
                    "geometric_center_scatter_p95_px"
                ),
                "center_fusion_source": summary.get("center_fusion_source"),
                ...
                "observations": observations,
                "error_counts": error_counts,
                "accepted_frame_indices": summary.get("accepted_frame_indices"),
            }
    ...
    if direct_strategy:
        capture_diagnostics = {
            ...
            "capture_elapsed_s": (
                ...
                else float(max(0.0, time.monotonic() - capture_started_monotonic))
            ),
            "capture_timed_out": bool(capture_timed_out),
            "settle_discard_timeout_s": settle_discard_timeout_s,
            "settle_discard_timed_out": bool(settle_discard_timed_out),
            "max_consecutive_frame_failures": max_consecutive_frame_failures,
            "max_consecutive_frame_failures_reached": bool(
        ...
        for result in batch_results.values():
            if isinstance(result, dict):
                result.update(capture_diagnostics)
    ...
    if successful:
        ...
        for hole_id, result in successful.items():
            for item in result["observations"]:
                if (
                    ...
                    or (result.get("accepted_frame_indices") is not None
                        and item.frame_index not in result["accepted_frame_indices"])
                ):
                    continue
        ...
        if point_chunks:
            archive_path = run_dir / (
                f"{artifact_tag}batch_coarse_{height_label}_all_holes_pointcloud.npz"
            )
            try:
                with artifact_measure(
                    timing,
        ...
        for hole_id, result in successful.items():
            pointcloud_path = _save_coarse_pointcloud_image(
                ...
                hole_id,
                run_dir,
                observations=[item for item in result["observations"]
                              if result.get("accepted_frame_indices") is None
                              or item.frame_index in result["accepted_frame_indices"]],
                cache_entry=None,
                T_base_camera=T_base_camera,
                intrinsics=last_intrinsics,
                rgb_path=latest_overlay_path,
                source_label=f"batch_coarse_{height_label}mm",
                timing=timing,
            )
    ...
    batch_results["_batch_metadata"] = {
        ...
        "target_height_mm": capture_height_mm,
        "height_label": f"{height_label}mm",
        "strategy": "coarse_direct_pointcloud_only" if direct_strategy else "shared_coarse",
        "max_group_size": int(getattr(cfg, "batch_coarse_max_group_size", 0) or 0),
    }
    notify_progress(
        "complete",
        captured_frame_count=len(frame_records),
        successful_hole_count=len(successful),
        capture_stop_reason=capture_stop_reason,
    )
    ...
    *,
    ...
    source_xy_by_hole = {
        int(hole["hole_id"]): np.asarray(
            (
                hole["coarse_center_base_mm"]
                if hole.get("coarse_center_base_mm") is not None else
    ...
    with timing.measure(
        ...
        for frame_index in range(max_attempts):
            ...
            for hole in selected_holes:
                ...
                with visual_measure(
                    timing,
                    f"batch_fine/ellipse_fitting_hole_{hole_id:02d}",
                    "ellipse_fitting",
                    frame_index=int(frame_index),
                    hole_id=int(hole_id),
                ):
            ...
            for hole_id in hole_ids:
                ...
                if _fine_burst_stable(
                    ...
                    min(
                        ...
                        float(batch_cfg.max_fine_center_scatter_p95_px),
                    ),
                ):
            ...
            if joint_enabled:
                ...
                for hole in selected_holes:
                    ...
                    try:
                        with visual_measure(
                            timing,
                            f"batch_fine/coordinate_transform_hole_{hole_id:02d}",
                            "coordinate_transform",
                            frame_index=int(frame_index),
                            hole_id=int(hole_id),
                        ):
                ...
                try:
                    with visual_measure(
                        timing,
                        "batch_fine/joint_fusion_quality",
                        "fusion_quality",
                        frame_index=int(frame_index),
