...
def fit_plane_model(points: np.ndarray) -> tuple[np.ndarray, np.ndarray, float, np.ndarray]:
    """Return one consistent plane (normal, anchor, RMSE, original-row mask)."""
    ...
    if keep.sum() >= 12:
        ...
        raise ValueError("平面拟合有效内点不足")
    mask = np.zeros(len(finite), dtype=bool)
    mask[np.flatnonzero(finite)[keep]] = True
    return normal / np.linalg.norm(normal), center, float(np.sqrt(np.mean(residual ** 2))), mask


def fit_plane(points: np.ndarray) -> tuple[np.ndarray, float]:
    normal, _, rmse, _ = fit_plane_model(points)
    return normal, rmse
