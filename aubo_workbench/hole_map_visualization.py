...
def _iter_payload_holes(payload: dict[str, Any]):
    """遍历普通粗地图 holes 和六扇区粗地图 sectors[*].holes。"""
    holes = payload.get("holes")
    if isinstance(holes, dict):
        yield from holes.values()
        return
    ...
    for sector in sectors.values():
        if not isinstance(sector, dict):
            continue
        sector_holes = sector.get("holes")
        if isinstance(sector_holes, dict):
            yield from sector_holes.values()
...
def _write_center_ply(path: Path, payload: dict[str, Any]) -> int:
    ...
    try:
        ...
        with temporary.open("w", encoding="ascii", newline="\n") as handle:
            ...
            for point, hole_id in records:
                red, green, blue = 150, 150, 150
                handle.write(
                    f"{point[0]:.5f} {point[1]:.5f} {point[2]:.5f} "
                    f"{red} {green} {blue} {hole_id} 0\n"
                )
        temporary.replace(path)
    finally:
        temporary.unlink(missing_ok=True)
...
def _panel_projection(
    ...
    for _, coarse in centers:
        all_values.append(np.asarray([coarse[[horizontal_axis, vertical_axis]]]))
    ...
    marker_points = np.asarray([
        np.asarray(coarse)[[horizontal_axis, vertical_axis]]
        for _, coarse in centers
    ], dtype=np.float64)
    if len(marker_points):
        marker_pixels = pixel(marker_points)
        for (hole_id, coarse), marker in zip(centers, marker_pixels):
            ...
            cv2.drawMarker(image, (gx, gy), (80, 80, 80), cv2.MARKER_CROSS, 14, 2, cv2.LINE_AA)
...
def _write_preview(
    ...
    *,
    ...
    cv2.putText(canvas, preview_policy, (48, 975), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (60, 60, 60), 1, cv2.LINE_AA)
