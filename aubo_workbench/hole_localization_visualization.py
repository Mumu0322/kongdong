...
def _save_grouping_plan_visualization(
    ...
    *,
    ...
    summary = "  ".join(
        ...
        + (
            f" {str(group[0].get('group_boundary_class', group[0].get('boundary_class', ''))).upper()}"
            if group and group[0].get("group_boundary_class", group[0].get("boundary_class"))
            else ""
        )
...
def _save_batch_fine_final_result_overlay(
    ...
    *,
    ...
    for result in final_results:
        ...
        fused_point_value = result.get("batch_fine_pointcloud_fused_point_base_mm")
        fused_point = (
            None if fused_point_value is None else
            np.asarray(fused_point_value, dtype=np.float64).reshape(3)
        )
        ...
        fused_px = projected(fused_point)
        ...
        if fused_px is not None:
            p = tuple(np.rint(fused_px).astype(int))
            cv2.drawMarker(view, p, (255, 0, 255), cv2.MARKER_STAR, 24, 2, cv2.LINE_AA)
            cv2.putText(view, f"H{hole_id} PC-FUSED", (p[0] + 8, p[1] - 10),
        ...
        records.append({
            ...
            "pointcloud_fused_point_base_mm": fused_point,
            "pointcloud_fused_point_px": fused_px,
    ...
    cv2.putText(
        view, "red+=RAW FINE  green=JOINT  magenta*=PC-FUSED  cyan=TARGET  yellow x=TCP",
