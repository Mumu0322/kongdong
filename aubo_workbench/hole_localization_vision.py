...
from aubo_workbench.fitting import fit_plane, fit_plane_model, fit_sphere
from aubo_workbench.coarse_surface import ring_coverage, select_front_surface
...
COARSE_SURFACE_SELECTION_POLICY = "front_surface_plane_distance_v3"
COARSE_SURFACE_MODEL = "local_tangent_plane_front_surface_plane_distance_v3"
...
def hole_camera_point(
    ...
    *,
    ...
    if policy == COARSE_SURFACE_SELECTION_POLICY:
        ...
        # 保留0.10R安全距离；前景深度仅初始化平面，最终按平面距离
        # 分割，避免相机Z截断倾斜表面远侧。
    ...
    ring_pixels = np.column_stack((xx[ring], yy[ring]))
    pixels = ring_pixels
    valid_depth = np.isfinite(points).all(axis=1) & (points[:, 2] > 100.0) & (points[:, 2] < 3000.0)
    points, pixels = points[valid_depth], pixels[valid_depth]
    ...
    if policy == COARSE_SURFACE_SELECTION_POLICY:
        normal, plane_point, plane_rmse, selected_mask, front_surface_z = select_front_surface(
            raw_points, pixels, (ring_u, ring_v),
        )
        points = raw_points[selected_mask]
    else:
        front_surface_z = float(np.percentile(points[:, 2], front_percentile))
        selected_mask = points[:, 2] <= front_surface_z + surface_band_mm
        if selected_mask.sum() < max(80, int(len(points) * min_surface_fraction)):
            selected_mask = np.ones(len(points), dtype=bool)
        points = points[selected_mask]
        normal, plane_point, plane_rmse, inliers = fit_plane_model(points)
        selected_mask[np.flatnonzero(selected_mask)[~inliers]] = False
        points = raw_points[selected_mask]
    coverage = ring_coverage(pixels[selected_mask], (ring_u, ring_v))
    ...
    result = {
        ...
        "ring_coverage_ratio": coverage["coverage_ratio"],
        "ring_max_gap_deg": coverage["max_gap_deg"],
        "ring_sector_counts": coverage["sector_counts"],
        "surface_selection_method": "seeded_plane_distance" if policy == COARSE_SURFACE_SELECTION_POLICY else "depth_band",
    }
    if include_points:
        result["points_camera_mm"] = np.asarray(points, dtype=np.float32)
        result["raw_points_camera_mm"] = np.asarray(raw_points, dtype=np.float32)
        result["raw_pixels"] = pixels.astype(np.int32)
        result["ring_pixels"] = ring_pixels.astype(np.int32)
        result["surface_selected_mask"] = selected_mask
    return point, result
