    ...
    *,
    ...
    if not estimate_rotation:
        ...
        return {
            ...
            "max_point_correction_mm": translation_norm,
    ...
    point_corrections = np.linalg.norm(predicted_points - source, axis=1)
    max_point_correction = float(np.max(point_corrections[inlier_mask]))
    # A rigid transform's translation is defined at the base-frame origin. For
    # holes hundreds of millimetres away, a small yaw can therefore produce a
    # large translation parameter even though every observed hole only moves a
    # few millimetres. Gate the physical correction at the accepted holes.
    if max_point_correction > float(max_translation_mm):
        raise ValueError(
            f"共享精定位联合孔位修正超限：{max_point_correction:.3f}mm > "
            f"{float(max_translation_mm):.3f}mm"
        )
    ...
    return {
        ...
        "translation_norm_mm": translation_norm,
        "max_point_correction_mm": max_point_correction,
        "predicted_xy_by_hole": {
            hole_id: np.asarray(predicted_point, dtype=np.float64)
            for hole_id, predicted_point in zip(available, predicted_points)
        },
