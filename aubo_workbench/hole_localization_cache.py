...
def _cache_entry_from_observations(
    ...
    *,
    ...
    if summary.get("accepted_frame_indices") is not None:
        valid = [item for item in valid if item.frame_index in summary["accepted_frame_indices"]]
...
def _reuse_initial_pointcloud_geometry_for_batch_fine(
    ...
    """
    # 粗地图调用会把地图几何投影成“导航种子”。它没有测得当前帧点云，
    # 不能被误记成初始点云复用来源；当前帧的真实视觉只在后续260 mm
    # 精定位阶段采集。
    if bool(hole.get("coarse_map_seed")):
        return False
